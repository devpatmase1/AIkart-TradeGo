...
_SOURCE_ALIASES = {
    ...
    "tencent": ("tencent", "腾讯"),
    "tushare": ("tushare",),
    "yahoo": ("yahoo", "雅虎", "yfinance", "yahoo finance", "yahoofinance"),
    "yfinance": ("yfinance", "yahoo", "雅虎", "yahoo finance", "yahoofinance"),
}
...
_CURRENCY_ALIASES = {
    "USD": ("usd", "us$", "$", "dollar", "dollars", "美元", "美金"),
    "CNY": ("cny", "cnh", "rmb", "人民币", "¥", "￥"),
    "HKD": ("hkd", "hk$", "港元", "港币"),
    "KRW": ("krw", "韩元", "韩圜", "₩"),
    "INR": ("inr", "₹", "rs", "rupee", "rupees", "印度卢比", "卢比"),
...
class GroundingLedger:
    ...
    def safe_fallback(self) -> str:
        ...
        if price_records:
            ...
            joined = "; ".join(facts)
            return (
                "I verified the observed market data from tool evidence. "
        ...
        if issue_codes & {
            ...
            return (
                "My previous answer was verified against market tool records: it cited figures "
                "that were not retrieved through a tool during this session. Please re-run "
                "with an explicit request to fetch market data first."
            )
        return (
            "I could not safely lock the instrument identity or price evidence. "
            "Please confirm the candidate symbol and market venue."
        )
    ...
    def _validate_price_provenance(
        ...
        mentioned = [
            ...
            or symbol.casefold() in folded
            or ("." in symbol and symbol.rsplit(".", 1)[0].casefold() in folded)
            or ("-" in symbol and symbol.split("-", 1)[0].casefold() in folded)
        ]
        if not mentioned:
            issues.append(
                {
