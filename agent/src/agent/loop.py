            ...
            while iteration < self.max_iterations:
                ...
                for attempt in range(max_stream_retries):
                    try:
                        ...
                        if not exc.retryable or attempt >= max_stream_retries - 1:
                            raise
                        err_str = str(exc)
                        delay = _stream_retry_delay_s() * (attempt + 1)
                        if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str or "rate_limit" in err_str.lower():
                            delay_match = re.search(r"(?:retry in|retryDelay'?: '?)([0-9.]+)", err_str, re.IGNORECASE)
                            if delay_match:
                                delay = max(float(delay_match.group(1)) + 1.0, 5.0)
                            else:
                                delay = max(delay, 20.0 * (attempt + 1))
                        logger.warning(
                            "Provider stream failed (iter %s, attempt %s/%s), retrying in %.1fs: %s",
                            current_iter,
                            attempt + 1,
                            max_stream_retries,
                            delay,
