  ...
  return (
    <div className="flex h-screen bg-background rtl:flex-row-reverse">
      ...
      <aside
        ...
        className={cn(
          "max-md:w-12 border-e border-border/50 bg-card/80 backdrop-blur-2xl flex flex-col shrink-0 transition-all duration-200 overflow-visible z-20 shadow-sm",
          collapsed ? "w-12" : "w-64"
        )}
      >
        {/* Brand */}
        <div className={cn("border-b border-border/50", collapsed ? "p-2 flex justify-center" : "px-4 py-3 max-md:p-2 max-md:flex max-md:justify-center")}>
          <Link
            to="/"
            aria-label="aiKart"
            className={cn("flex items-center transition-transform hover:scale-[1.02]", collapsed ? "justify-center" : "max-md:justify-center")}
          >
            {collapsed ? (
              <BrandMark variant="icon" className="h-7 w-7 shrink-0" />
            ) : (
              <BrandMark variant="full" className="h-7 w-auto max-w-[150px] shrink-0" />
            )}
          </Link>
        </div>
        ...
        <nav
          ...
          className={cn("space-y-1.5", collapsed ? "p-1.5" : "p-3 max-md:p-1.5")}
        >
          {NAV.map(({ to, icon: Icon, label }) => {
            const text = label;
            const isActive = to === "/" ? pathname === "/" || pathname.startsWith("/agent") : pathname.startsWith(to);
            return (
              <Link
                key={to}
                to={to}
                aria-label={text}
                className={cn(
                  "flex items-center rounded-full text-[13px] transition-all duration-200",
                  collapsed ? "justify-center px-2 py-2" : "gap-3 px-3.5 py-2 max-md:justify-center max-md:px-2",
                  isActive
                    ? "bg-primary text-primary-foreground font-semibold shadow-md shadow-primary/25 translate-x-0.5"
